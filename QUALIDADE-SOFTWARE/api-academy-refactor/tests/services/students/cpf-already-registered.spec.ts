...
describe('Testes de unidade para o método cpfAlreadyRegistered', () => {
  test('Deve retornar false se o CPF não existir', async () => {
    // Arrange - Preparar o teste
    prismaMock.student.findUnique.mockResolvedValue(null);
    const sut = new StudentService();

    // Act     - Rodar o teste
    const result = await sut.cpfAlreadyRegistered('any_cpf');

    // Assert  - Verificar as asserções
    expect(result).toBe(false);
  });

  test('Deve retornar true se o CPF existir', async () => {
    // Arrange - Preparar o teste
    prismaMock.student.findUnique.mockResolvedValue(fakeStudent);
    const sut = new StudentService();

    // Act     - Rodar o teste
    const result = await sut.emailAlreadyRegistered('any_cpf');
